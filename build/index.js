...
const server = new index_js_1.Server({ name: "android-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
// ── Helper: wrap result ──
function ok(text) {
...
function prep(serial) {
    const p = (0, device_1.getDevicePrefix)(serial);
    (0, device_1.checkDevice)(p);
    return p;
}
// ── Helper: capture screen summary ──
function screenSummary(p) {
    try {
        const xml = (0, device_1.captureUIXml)(p);
    ...
    try {
        switch (name) {
            ...
            case "mobile_device_list": {
                const a = args;
                if (a.select) {
                    const devices = (0, device_1.listDevices)();
                    ...
                    (0, device_1.setDeviceSerial)(a.select);
                }
                const devices = (0, device_1.listDevices)();
                const selected = (0, device_1.getSelectedSerial)();
                if (devices.length === 0)
                    return ok("No devices connected");
            ...
            case "mobile_screenshot": {
                ...
                const wake = (0, device_1.adb)([...p, "shell", "dumpsys", "power"]);
                ...
                const rawPng = (0, device_1.captureScreenshotRaw)(p);
            ...
            case "mobile_app": {
                ...
                if (a.action === "install") {
                    if (!a.file)
                        return err("file is required for install");
                    if (!fs.existsSync(a.file))
                        return err(`File not found: ${a.file}`);
                    const r = (0, device_1.adb)([...p, "install", "-r", a.file]);
                    if (r.status === 0) {
                    ...
                    const r = (0, device_1.adb)([...p, "shell", "pm", "list", "packages", "--user", "0"]);
                    if (r.status !== 0)
                        return err(`Failed to list packages: ${(r.stderr || r.stdout || "").trim()}`);
                    let lines = r.stdout.split("\n").filter((l) => l.startsWith("package:")).map((l) => l.replace("package:", "").trim());
                    if (a.package) {
                        const pkg = a.package;
                        lines = lines.filter((l) => l.toLowerCase().includes(pkg.toLowerCase()));
                    }
                    result = lines.length === 0 ? (a.package ? `No packages match "${a.package}"` : "No packages installed") : lines.join("\n");
                }
                else {
                    if (!a.package)
                        return err("package is required for start/force-stop/delete");
                    if (a.action === "start") {
                        const r = (0, device_1.adb)([...p, "shell", "monkey", "-p", a.package, "1"]);
                        ...
                        (0, device_1.adb)([...p, "shell", "am", "force-stop", a.package]);
                        result = `Force-stopped ${a.package}`;
                    }
                    else if (a.action === "delete") {
                        const r = (0, device_1.adb)([...p, "uninstall", a.package]);
                        if (r.status !== 0)
                            return err(`Failed to uninstall ${a.package}: ${(r.stderr || r.stdout || "").trim()}`);
            ...
            case "mobile_gesture": {
                ...
                if (a.text) {
                    const xml = (0, device_1.captureUIXml)(p);
                    const elements = (0, uixml_1.parseUIElements)(xml);
                    const el = (0, uixml_1.findElement)(elements, { text: a.text });
                    if (!el)
                        return err(`Element with text "${a.text}" not found`);
                    x = el.center.x;
                    y = el.center.y;
                }
                const step = {
            ...
            case "mobile_gesture_combo": {
                const a = args;
                const p = prep(a.serial);
                (0, gesture_1.runSteps)(p, a.steps);
            ...
            case "mobile_gesture_macro": {
                const a = args;
                const p = prep(a.serial);
                const display = (0, device_1.adb)([...p, "shell", "wm", "size"]);
                ...
                if (!steps)
                    return err(`Unknown macro: ${a.macro}`);
                (0, gesture_1.runSteps)(p, steps);
            ...
            case "mobile_button": {
                ...
                if (!key)
                    return err(`Unknown button: ${a.button}`);
                (0, device_1.adb)([...p, "shell", "input", "keyevent", key]);
            ...
            case "mobile_type": {
                ...
                const p = prep(a.serial);
                if (a.clear_first) {
                    (0, device_1.adb)([...p, "shell", "input", "keyevent", "KEYCODE_MOVE_END"]);
                    // Select all + delete
                    for (let i = 0; i < 50; i++) {
                        (0, device_1.adb)([...p, "shell", "input", "keyevent", "KEYCODE_DEL"]);
                    }
                }
                (0, device_1.adb)([...p, "shell", "input", "text", a.text]);
            ...
            case "mobile_unlock_screen": {
                ...
                (0, device_1.adb)([...p, "shell", "input", "swipe",
                    ...
                    "300"]);
                (0, gesture_1.busySleep)(1000);
                // Enter PIN if provided
                if (a.pin) {
                    (0, device_1.adb)([...p, "shell", "input", "text", a.pin]);
                    (0, gesture_1.busySleep)(300);
                    (0, device_1.adb)([...p, "shell", "input", "keyevent", "KEYCODE_ENTER"]);
                }
            ...
            case "mobile_wait": {
                ...
                while (Date.now() - start < timeout) {
                    const xml = (0, device_1.captureUIXml)(p);
                    const elements = (0, uixml_1.parseUIElements)(xml);
                    const found = (0, uixml_1.findElement)(elements, {
                        text: a.text,
                        resourceId: a.resource_id,
                        className: a.className,
                        description: a.description,
                    });
                    if (a.disappear && !found)
                        return ok(`Element disappeared (waited ${Date.now() - start}ms)`);
                    if (!a.disappear && found)
                        return ok(`Element found (waited ${Date.now() - start}ms)`);
                    (0, gesture_1.busySleep)(500);
                }
            ...
            case "mobile_state": {
                const a = args;
                const p = (0, device_1.getDevicePrefix)(a.serial);
                const battery = (0, device_1.adb)([...p, "shell", "dumpsys", "battery"]);
                ...
                const display = (0, device_1.adb)([...p, "shell", "wm", "size"]);
                const density = (0, device_1.adb)([...p, "shell", "wm", "density"]);
                const wake = (0, device_1.adb)([...p, "shell", "dumpsys", "power"]);
                const awake = wake.stdout.includes("mWakefulness=Awake") ? "awake" : "sleeping";
                const rotation = (0, device_1.adb)([...p, "shell", "dumpsys", "window", "rotations"]);
                const rotM = /Rotation: (\d)/.exec(rotation.stdout);
                const rotMap = { "0": "portrait", "1": "landscape", "2": "reverse portrait", "3": "reverse landscape" };
                const foreground = (0, uixml_1.getForegroundPackage)(p);
                const wifi = (0, device_1.adb)([...p, "shell", "dumpsys", "wifi"]);
            ...
            case "mobile_activity_manager": {
                ...
                const intentParts = [];
                if (a.action)
                    intentParts.push("-a", a.action);
                if (a.data_uri)
                    intentParts.push("-d", a.data_uri);
                if (a.package)
                    intentParts.push(a.package);
                if (a.class_name) {
                    if (a.package)
                        intentParts.push("-n", a.class_name.startsWith(a.package) ? a.class_name : `${a.package}/${a.class_name}`);
                    else
                        intentParts.push("-n", a.class_name);
                }
                if (a.extras) {
                    try {
                        ...
                        for (const [k, v] of Object.entries(parsed)) {
                            if (typeof v === "string")
                                intentParts.push("--es", k, v);
                            else if (typeof v === "number")
                                intentParts.push("--ei", k, String(v));
                            else if (typeof v === "boolean")
                                intentParts.push("--ez", k, String(v));
                        }
                    }
                    catch {
                ...
                (0, device_1.adb)([...p, "shell", "am", "start", ...intentParts]);
                return ok(`Activity started: ${intentParts.join(" ") || "(default launcher)"}`);
            }
            // ── Restart ADB ──
            case "mobile_restart_bridge": {
                (0, device_1.adb)(["kill-server"]);
                (0, gesture_1.busySleep)(2000);
                (0, device_1.adb)(["start-server"]);
                (0, gesture_1.busySleep)(1000);
            ...
            case "fdroid_search": {
                ...
                if (!res.ok)
                    return err(`F-Droid search error: ${res.status}`);
                ...
                const descs = [];
                let m;
                while ((m = pkgRegex.exec(html)) !== null && pkgs.length < limit)
                    pkgs.push(m[1]);
                while ((m = nameRegex.exec(html)) !== null && names.length < limit)
                    names.push(m[1].trim());
                while ((m = descRegex.exec(html)) !== null && descs.length < limit)
                    descs.push(m[1].trim());
                if (pkgs.length === 0)
                    return ok(`No F-Droid results for "${a.query}"`);
            ...
            case "fdroid_download": {
                ...
                const res = await fetch(url);
                if (!res.ok)
                    return err(`F-Droid package page error: ${res.status}`);
                const html = await res.text();
                ...
                if (!apkMatch)
                    return err(`No APK download link found for ${a.package_name}`);
                const apkUrl = apkMatch[1];
                const apkRes = await fetch(apkUrl);
                if (!apkRes.ok)
                    return err(`Download failed: ${apkRes.status}`);
            ...
            case "password_manager": {
                ...
                if (a.action === "store") {
                    if (!a.app || !a.username || !a.password)
                        return err("app, username, and password are required for store");
                    ...
                    if (!a.app)
                        return err("app is required for get");
                    const pwPath = path.join(pwDir, `${a.app}.json`);
                    if (!fs.existsSync(pwPath))
                        return err(`No credentials stored for ${a.app}`);
